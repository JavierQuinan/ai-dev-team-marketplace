...
- **Progressive disclosure is mandatory, not optional.** `SKILL.md` files stay under ~250 lines and link to `references/` for shared policy (evidence rules, safety rules, context-recovery strategy) instead of restating it. Rationale and mechanics are recorded in [../architecture/token-efficiency.md](../architecture/token-efficiency.md).
...
- **Eval-driven from the start.** Each of the ten skills ships with scenario-based evals under `tests/evals/`, validated by schema in CI, rather than relying only on manual testing before merge.
- **Security-by-default, autonomy-with-limits.** A shared `references/evidence-and-safety.md` policy — no unverified claims of success, explicit confirmation required before irreversible actions (force-push, hard reset, destructive migrations, production deploys, merges, secret rotation) — is referenced by every skill instead of being trusted to be remembered ad hoc.
