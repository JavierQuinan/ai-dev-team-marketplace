...
1. **Separate the current deployment shape from the requested target — they answer different questions.**
   - **Current deployment evidence** (what the repo shows *today*): look for `vercel.json`/`.vercel/project.json` (Vercel), `Dockerfile`/`docker-compose*`/`compose*` (Docker), systemd units/`nginx`/`caddy` config (VPS), Kubernetes manifests/Helm charts (K8s), `Procfile`, CI deploy jobs, or deployment docs naming the platform. A framework being *compatible* with a platform (e.g. Next.js auto-detecting on Vercel) is never itself evidence of the current target — that same app could just as legitimately run on Docker, a VPS, or elsewhere; only platform-specific config/CI/docs count.
   - **Requested target** (what the user asks to plan *for*): when the user explicitly names a target ("despliega esto en Vercel", "planifica cómo migrar esto a Docker"), that target governs the plan — repo evidence about the *current* shape never overrides an explicit instruction about the *desired* one. Label it `TARGET: <X>` / `SOURCE: USER-PROVIDED` / `REPO SUPPORT: verified` (if current evidence also confirms it) or `NOT VERIFIED — current repo shows <Y>` (if it doesn't, or shows something else). Never present a user-requested target as repo-confirmed when it isn't.
   - **When they disagree** (e.g. current evidence is Docker, requested target is Vercel): say so explicitly — never silently plan for the current shape instead of the requested one, and never silently drop the current evidence. Produce the plan for the *requested* target, cite the current evidence as context, and add a migration/config-gap note naming what has to change to get there.
   - **If no requested target exists**, use current repo evidence as the target. If neither a requested target nor repo evidence exists, state **DEPLOY TARGET NOT VERIFIED** and produce the generic bounded plan (see `references/deployment-planning.md`) — never default to inventing Vercel/AWS/Kubernetes.
...
1. **Target/platform evidence** — the current deployment evidence found, the requested target and its source (repo-verified vs. user-provided) if one was given, any conflict between the two named explicitly with a migration/config-gap note, or `DEPLOY TARGET NOT VERIFIED` if neither exists.
...
- Platform can't be determined from evidence and no target was requested → say `DEPLOY TARGET NOT VERIFIED` and produce the generic bounded plan; never guess a default platform.
- User requests a target that conflicts with the repo's current deployment evidence (e.g. repo is Docker, user asks to plan Vercel) → the requested target governs the plan produced; state both the current evidence and the requested target explicitly, name the conflict, and add a migration/config-gap note — never silently substitute the current shape for what was asked, and never claim the requested target is repo-verified when the evidence actually shows something else.
