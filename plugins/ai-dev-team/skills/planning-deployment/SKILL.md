...
1. **Detect the platform from repo evidence** — never assume. Look for `vercel.json`/build config (Vercel), `Dockerfile`/`docker-compose*`/`compose*` (Docker), systemd units/`nginx`/`caddy` config (VPS), Kubernetes manifests/Helm charts (K8s), `Procfile`, CI deploy jobs, or deployment docs. Evidence beats a user's stated platform, which beats inference. If no platform evidence exists, state **DEPLOY TARGET NOT VERIFIED** and produce the generic bounded plan (see `references/deployment-planning.md`) — never default to inventing Vercel/AWS/Kubernetes.
