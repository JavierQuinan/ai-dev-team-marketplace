...
3. **Draft the plan**, covering only the sections that actually apply:
   ...
   - **API impact** — new/changed endpoints or contracts. When a request/response contract is actually changing, identify the OLD and NEW contract and the direction (request vs. response — see [references/api-contract-review.md](../../references/api-contract-review.md) for the compatibility model) before proposing a strategy; a breaking change gets a concrete rollout strategy in the plan (additive rollout, deprecate-then-remove, dual-field transition, parallel versions, or a coordinated rollout — only when every affected consumer, not just the provider, is evidenced as under coordinated control and able to move in lockstep; a single deploy pipeline or monorepo is evidence toward that, never sufficient by itself if an external or independent consumer exists) — never a blanket version bump when the change is actually compatible/additive.
