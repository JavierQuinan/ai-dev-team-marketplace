...
**"Full team" never means "launch all 10 agents."** Default to doing a step inline; spawn a subagent only when it earns its isolated-context cost (see `references/orchestration-budget.md` for the full model: task size bands from Tiny (0 agents) to Large (3–6), the inline-first principle, task packets, no-duplicate-discovery, and the one-writer-per-area ownership ledger). A ten-agent fanout for a small task is the failure mode this budget exists to prevent, not a sign of thoroughness.
...
- Change touches multi-tenant data, auth, tenancy, or payments → REVIEW must include `auditing-security`, no exceptions.
