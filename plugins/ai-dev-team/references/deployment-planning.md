...
Deep, platform-specific patterns for `planning-deployment`, loaded once a target has been identified from repo evidence (or ruled unverifiable). None of these ever get executed by the skill itself — every command shown here is a *human-run* command in the plan, never a `Bash` call this skill makes.
