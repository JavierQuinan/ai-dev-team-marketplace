...
- **Nullable column with a default**: the relevant distinction since Postgres 11 is volatility, not constant vs. function-call. A default whose expression is `IMMUTABLE` or `STABLE` — a literal constant, but also a stable function such as `now()`/`current_timestamp` (constant within the evaluating transaction) — can use the fast-default path (metadata-only, no rewrite). A default marked `VOLATILE` — `random()`, `clock_timestamp()`, a sequence via `nextval()`, or any function explicitly declared `VOLATILE` — still requires per-row evaluation and a table rewrite, because there's no single value that can represent it for every pre-existing row. Check the actual default expression's declared volatility (`\df+` in `psql`, or the function's own `CREATE FUNCTION` volatility marker), not just whether it happens to be a function call.
...
- With RLS enabled, the *absence* of an applicable policy for a command is default-deny, not an opening — a table with only a `SELECT` policy and no `UPDATE` policy correctly blocks `UPDATE` entirely, it doesn't leave it unfiltered. The real risk is the opposite: an applicable policy that's over-broad. Audit every policy on `SELECT`/`INSERT`/`UPDATE`/`DELETE` individually and check each one for: `USING (true)` or an equivalent tautology; a permissive `FOR ALL` policy covering more commands than intended; a tenant/ownership predicate that's missing, wrong, or checks the wrong column; a missing or unsafe `WITH CHECK` on `INSERT`/`UPDATE` (a `USING` clause alone doesn't constrain the *new* row values being written); and, when multiple permissive policies apply to the same command, whether their combined (OR'd) effect is broader than any single policy suggests.
